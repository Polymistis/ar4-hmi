...
- `AR4.py` identifies host source version 6.7. The imported Teensy baseline identifies version 6.7.1; the active tracked derivative identifies version `6.7.1-ar4hmi.2` and advertises `JT_WRIST_CONFIG_V1`, `GCODE_DIRECTORY_FRAMING_V1`, `GCODE_DELETE_IDENTITY_V1`, and `GCODE_WRITE_IDENTITY_V1`.
...
- G-code playback, deletion, and conversion validate an ASCII path component
  ...
  tracked Teensy `6.7.1-ar4hmi.2` protocol has no `TF` handler.
...
- The paired line-oriented Teensy `6.7.1-ar4hmi.2` compatibility unit advertises `JT_WRIST_CONFIG_V1`, `GCODE_DIRECTORY_FRAMING_V1`, `GCODE_DELETE_IDENTITY_V1`, and `GCODE_WRITE_IDENTITY_V1` and compiles for `teensy:avr:teensy41` with PJRC core 1.62.0, bundled SdFat 2.1.2, and ModbusMaster 2.0.1. The compile regression parses the verbose dependency report and requires the active SPI and SdFat folders under the selected Teensy platform; `docs/hardware-free-verification-2026-07-19.md` records the exact no-upload result and establishes no live-arm behavior or correlated JSON readiness.
