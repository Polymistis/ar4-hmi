...
- J1/J2 homing verification must confirm switch release within the lesser of
...
- `Shutdown Position` keeps J3-J6 at the canonical start values and uses the
  J1/J2 switch coordinates reported by the connected Teensy after successful
  homing under the active controller frame. Each requested reference is
  ...
  commands before querying the controller, so J1 and J2 require fresh homing
...
- Start and shutdown targets are validated against active calibration before
  admission.
- `HOME_REFERENCE_V1` framing, paired host/firmware invalidation,
  post-homing commit, startup capability discovery and invalid-state
  synchronization, and host parsing have deterministic coverage. The start
  target remains checked against `Home.ar4`.
