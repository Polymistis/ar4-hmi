...
constexpr size_t kIdentityJsonCapacity = 640;
...
inline bool build_identity_json(
    ...
    const char* protocol_capability,
    char* output,
    size_t output_capacity
) {
    if (output == nullptr || output_capacity == 0) return false;
    output[0] = '\0';
    size_t index = 0;
    return append_json_text("{\"DriverModel\":\"", output, output_capacity, index)
        ...
        && append_json_text("\",\"ProtocolCapabilities\":[\"", output, output_capacity, index)
        && append_identity_json_value(protocol_capability, output, output_capacity, index)
