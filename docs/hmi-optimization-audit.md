...
- Owned framed-response readers remove only a required LF and an optional
  preceding CR. Validation separates the 4096-byte payload limit from the
  4098-byte CRLF-capable frame limit, so a maximum-length payload remains valid
  with either supported delimiter. Leading or trailing payload whitespace,
  extra delimiters, non-ASCII bytes, and queued data outside an
  operation-specific follow-up contract fail the protocol contract and
  quarantine the affected transport. Terminal ownership requires a complete
  bounded quiet interval; insufficient remaining response time quarantines the
  ...
  `6.7.1-ar4hmi.10` derivative atomically checks the stop latch and input at the
  ...
  output without writing USB serial data, and main-loop completion emits
  pending `EB` only at a safe frame boundary. Controller and Modbus wait delays
  poll the E-stop latch and return from the active response scope after
  consuming the command. A pre-terminal interruption emits `ER` before the
  scoped `EB`; a post-terminal interruption publishes `EB` without another
  terminal. Modbus transactions remain bounded by the selected library before
  the next latch poll. Automated coverage and compilation do not establish
  live response latency. A blocked command reserves the
  correlated `EA` response against pending `EB` publication. Admission and
  loop-response ownership retire together with interrupts disabled, and
  generation comparison prevents an older response from clearing a newer
  assertion.
  Telemetry-enabled `RJ` ownership still spans coordinated drive through
  ...
  wrapper preserve unread input, classify stop frames before operation-specific
  interim callbacks, and own ordinary terminals, standalone `EA`, and
  terminal/`EB` pairs. An acknowledged `LC`, `LJ`, or `LT` exchange derives a
  finite post-terminal probe deadline from the configured control-response
  bound before rejecting an unrequested ordinary terminal. A paired `EB` is
  published and authoritative; an absent or invalid pair triggers fail-safe
  `S` and transport quarantine. An immediately available `EA` after `EB` remains
  response-owned; an `EB` without a follow-up byte in the bounded probe closes
  the telemetry connection before ownership release. When no command owns the
  main transport, a zero-write monitor checks only the local pySerial queued-byte
  count and consumes a standalone `EB` through a bounded worker under
  exclusive emergency ownership. The worker publishes and quarantines the
  stop before Tk result transfer; Tk releases the emergency transport owner
  before the separately queued stop presentation, while shutdown prevents
  final close until queued or retained presentation settles. The monitor
  remains admissible while shutdown drains. Another idle frame quarantines and
  closes the controller connection. Stop publication is mandatory for each accepted
  physical-stop exchange. Publication reads the complete auxiliary profile
  under the same configuration/stop-state lock order used by configuration
  commits, then atomically reserves a retryable auxiliary `STOP` with manual
  auxiliary admission blocked and queued work cancelled before Tk receives the
  event. A configuration commit racing the stop either rejects against the
  latch or becomes visible as one complete profile; physical-stop latching does
  not wait for auxiliary transport ownership. The request remains owned until
  acknowledgement; configured stop control remains admissible after normal
  shutdown admission closes, and final serial closure waits for pending or
  active stop ownership plus queued or retained stop presentation.
  Failed Tk pose, status, or invalidation work remains retained for retry. The
  joint dispatcher does not apply the same stop again.
  Main-controller events from joint, legacy, and G-code storage exchanges use
  connection-epoch and confirmed-position-generation correlation to prevent a
  delayed stop from replacing a newer pose. A retained presentation from a
  prior controller session is retired diagnostically and cannot re-latch a
  fault-free replacement controller.
  Manual Modbus events remain request-correlated. Unmatched combinations
  quarantine the transport. Startup requires the advertised
  `ESTOP_ADMISSION_V1` capability. Speculative spline acknowledgements remain a
  multi-frame surface. Compilation and hardware-free contract verification
  cover this framing revision in the
  [dated no-upload record](hardware-free-verification-2026-07-19.md), while
  authorized live-arm verification remains pending.
- Live joint, Cartesian, and tool jog callbacks no longer sleep or read serial data. The existing worker owns the live command, consumes the firmware's framed blank acknowledgement, observes a thread-safe stop request, writes `S\n`, and consumes the final position response. Firmware polls a byte between live segments and accepts only an exact LF- or CRLF-terminated `S` control frame; an overlength or other complete frame exits through one error response instead of being treated as a successful stop. Initial acknowledgement uses the dedicated `SERIAL_LIVE_ACK_TIMEOUT_SECONDS` watchdog and post-token completion uses the command-specific deadline derived from encoded timing and configured full-travel bounds. Seconds and millimetres-per-second selections are normalized to Percent for live modes because those units do not provide predictable stop-polling cadence. The response deadline remains suspended while the acknowledged control remains held, but every empty read still verifies that the port remains open. Closure or cancelled-read termination enters fail-safe stop and quarantine handling instead of retaining ownership indefinitely. Cancellation admission is rechecked under the serial write lock after the bounded pending-stop probe and without discarding unread input; cancellation before transmission writes neither the live command nor `S\n`. Premature terminal data or another framing-uncertain failure after transmission writes the stop token where supported, quarantines the transport, attempts a verified close, and requires an explicit reconnect. Failed physical close retains the poisoned handle under application cleanup ownership. Temporary per-read timeout restoration is attempted across return and exception paths, and restoration failure triggers the same quarantine. pySerial documents timeout-controlled line reads and `close()` changing `is_open` to false in the [short introduction](https://pyserial.readthedocs.io/en/latest/shortintro.html).
...
- Visible manual Modbus read and write controls validate and canonicalize the
  ...
  firmware and host exchange one scalar response value. A request-scoped
  worker owns the main transport and
  shutdown-activity lease through bounded response parsing and Tk result
  application. Manual controller I/O shares lock-protected admission with
  whole-program execution, G-code storage, and G-code conversion; either side
  rejects overlap through final cleanup settlement. `BE` and `BF` writes also
  reject under the shared stop-state locks while an E-stop latch,
  position-fault latch, or stop-settlement owner remains active. Overlapping
  panel requests reject without blocking Tk. Joint
  input entered during the exchange remains semantic deferred intent and is
  ...
  the response owner validates the terminal frame and an asynchronous E-stop
  frame in either order, a standalone correlated `EA` admission rejection, and
  an asynchronous `EB` immediately followed by `EA`. Each validated stop
  exchange snapshots the complete auxiliary profile under the shared
  configuration/stop-state lock order, atomically latches shared stop state,
  cancels queued manual auxiliary work, blocks new manual auxiliary admission,
  and reserves a request-correlated auxiliary `STOP` before publishing an
  independent Tk event. Auxiliary configuration commits use the same boundary
  and reject active E-stop, position-fault, or stop-settlement state. A failed
  `STOP` attempt restores the request to pending state until acknowledgement.
  The worker continues to drain owned framing or quarantine an unmatched
  combination. Request correlation prevents a stop event from settling a
  different manual operation. Failed pose rendering, status rendering, or
  motion invalidation retains the manual stop event ahead of its terminal
  result for retry. A stale retained presentation is diagnostic-only after a
  replacement controller reaches fault-free startup. The reported position is
  then applied on Tk, retains the stop alarm, requires connection closure before
  ownership release, and discards deferred motion.
  Joint motion writes preserve unread input. A late unowned `EB` is consumed
  by the zero-write idle monitor under exclusive emergency ownership after
  local queued-byte detection. Stop publication and quarantine precede Tk
  result transfer; transport ownership releases before the separately queued
  stop presentation, and shutdown waits for that presentation. Active
  command owners independently classify stop frames; any other idle frame
  quarantines and closes the main transport. Standalone `ER` responses and
  read-side `Modbus Error` responses remain explicit rejections. A `BE` or
  `BF` `Modbus Error`, a write-side `ER` paired with an asynchronous `EB`, or
  another failure after write transmission starts records an indeterminate
  external-device state, closes the connection, discards deferred motion, and
  requires reconnection plus device-state verification. The tracked firmware
  publishes the result-specific write terminal before post-transaction E-stop
  polling and reports an interrupted multi-register drive-service sequence as
  indeterminate. Drive-service completion also aggregates every register
  result. Failed connection close or ownership release remains in a retryable
  cleanup owner that blocks new panel work and application shutdown.
  Main-controller replacement also rejects while that cleanup owner remains
  pending.
  Physical resources may release before final presentation, but retained result
  ownership persists until rendering and a post-render connection-trust check both
  succeed.
  Presentation selection follows connection-trust and result-application
  evaluation, preventing normal completion from surviving a required close.
  Post-release connection loss detaches the stale handle, clears matching
  controller identity, invalidates pending joint intent, and renders a
  reconnection alarm.
  Deferred dispatch returns explicit idle, blocked, dispatched, or rejected
  state; only trusted idle settlement can report `SYSTEM READY`. Shared status
  updates and dispatch admission preserve active program-stop, E-stop, and
  position-fault reservations. Run and Step admission rejects those fault
  latches without allocating program ownership or clearing state. Only a bound,
  fault-free controller startup position clears the E-stop and position-fault
  latches; unsettled stop ownership remains blocking.
...
- The owned main-controller line-exchange path has a finite write timeout and command-specific finite response deadlines for bounded commands, derived from speed, acceleration, deceleration, and ramp values after conversion to the controller's binary32 representation, configured full-travel timing, and a safety margin. Teensy `PG` playback has no file-duration contract, so the serial worker retains response ownership until a terminal line or application shutdown instead of treating a fixed threshold as lost framing. Shutdown cancellation quarantines the host transport and attempts a verified close but does not claim physical-motion preemption. Pre-write calibration activity remains interruptible; committed calibration retains ownership until an applied terminal `LL` frame or explicit post-write quarantine and verified-close handling settles the operation. Legacy main-controller exchanges perform a bounded pre-write physical-stop probe without resetting unread input, then consume a stop-aware bounded framed or exact response under the shared dispatcher transport admission. A reported stop is published, quarantines and closes the controller connection, and rejects the caller before command-specific parsing; callers remain blocking until queue migration. Public callbacks that change controller position, calibration, or port selection acquire logical-motion admission before transport. `requestPos` and main-controller replacement are explicit position-recovery admissions. Main-controller replacement reserves the shared transport before close/open, commits `CAL['comPort']` only after fault-free startup position application, and retains ownership through failure cleanup. Auxiliary replacement or disablement keeps the requested port and board staged while close/open and handle binding complete. Transport replacement, live calibration, and UI publication share the physical-stop critical section; a stop admitted first preserves the prior binding and blocks replacement, while a later stop observes the verified replacement. Failed replacement restores the prior persisted configuration and UI selection. Any retained auxiliary handle remains stop-required even when persisted configuration is absent or damaged. Controller startup captures Tk-backed command state before launch, treats input or output buffer-reset failure as fatal, rechecks cancellation after stale-input reset and before write admission, and closes the failed connection; a worker attempts the optional auxiliary connection, performs bounded `UP`, `CE`, `SP`, and `RP` main-controller exchanges regardless of auxiliary availability, consumes unframed `UP` and `CE` acknowledgements through a bounded quiet boundary, normalizes CRLF on the framed `SP` acknowledgement, then Tk applies the typed position and visual-option result with the resulting connection status. Normal and forced position senders consume the complete framed `Done` acknowledgement and quiet boundary before a forced-pose path begins a separately owned `RP` exchange. An acknowledged forced target remains the reconnect `SP` source while `RP` fails or raises, and only a later fault-free authoritative position response clears that recovery target. Unavailable, disabled, or failed auxiliary startup closes and clears any pre-existing auxiliary handle before main-controller synchronization; failed cleanup aborts the startup result, retains the handle, and retries close rather than dropping ownership. Failed main-controller cleanup retains the port, activity lease, and serial reservation in a durable non-Tk retry owner, including simultaneous close and Tk-scheduler failure. A startup timeout dismisses the modal spinner and marks the connection attempt cancelled. Failed Tk scheduling no longer joins the worker; worker-side cleanup closes startup transports before releasing ownership. Shutdown closes the joint dispatcher, drains terminal events ahead of final persistence, and waits asynchronously for virtual work, retained cleanup, queued or retained generic stop presentation, owned transports, and tracked direct activity before serial close and GUI destruction.
- Startup performs an explicit stale-input reset before synchronization and
  never resets unread input between `HO`, `UP`, `CE`, `SP`, and `RP`.
  Every unbound startup exchange performs a bounded pending-stop probe,
  rechecks cancellation before write, then consumes a stop-aware exact or
  framed response. Queued or paired `EA` and `EB` frames latch and publish the
  stop before startup retry or generic failure handling.
- The worker-side physical-stop latch sets the request-scoped program
  cancellation event before Tk presentation. Run and Step modes check
  cancellation before row dispatch. A cancelled pending Step Reverse callback
  aborts row state and releases the matching program owner; only an
  already-released request is stale. Called-program workers load bounded
  regular `.ar4` leaf files, enqueue validated rows for request-scoped Tk
  application, and wait for the matching acknowledgement. The cancellation
  boundary orders that transition but is released before widget access, so
  concurrent Stop admission cannot form a worker/Tk lock cycle. Manual `.ar4`
  loading uses the same bounded file, row, line, UTF-8, and control-character
  contract and retains the selected source path for later editor writes.
  Program-issued calls retain leaf-only targets, resolve each child beside the
  current source, and preserve the exact caller source path for return
  navigation.
  Program-view application is transactional across rows, styles,
  selection, scroll state, path, current row, and called-program return state.
  Every `executeRow` command branch checks cancellation before local effects,
  and
  main and auxiliary transmissions recheck at the write boundary. `.ar4`
  `BH` and `BD` register reads use the canonical scalar builder; legacy
  multi-register rows fail before transmission with a scalar-row
  migration diagnostic. Serial, joint, virtual, calibration, program, and
  auxiliary result statuses share a stop-aware presenter, preventing later
  poll results from replacing a reserved E-stop or stop-settlement alarm.
  Manual, Xbox, and program auxiliary commands also share a write-boundary
  stop check under the manual auxiliary state lock. Physical-stop barrier
  publication acquires the same boundary, ordering every ordinary write
  commitment wholly before the barrier or after a rejecting stop check.
- Request-scoped program cancellation and startup timeout cancellation share
  ...
  playback reject cancellation before commitment while retaining response
  ownership after a committed write. Every main application-status label
  mutation passes through the stop-aware presenter.
  Program `SC` and `SO` rows use the canonical controller builder. Their
  framed exchanges admit `-1` and `-2` through the command-aware terminal
  classifier before final disposition. A `-1` or unrecognized post-write
  terminal quarantines the main connection and stops program advancement as
  an indeterminate external-device mutation; `-2` remains a definite
  rejection.
...
- Worker-run program motion acquires an exclusive request-scoped motion lease
  ...
  `Tr` without writing beyond the Cartesian pose buffer, but arc and
  ...
  Teensy `6.7.1-ar4hmi.10` parser has no `TF` command. Rejection leaves the
...
The shared stop-aware bounded exchange now covers the active calibration,
position, home-reference, G-code row, playback, and storage call paths instead
of resetting unread input or reading the active handle directly. Physical-stop
frames are classified before command-specific parsing, published for Tk
application, and force connection quarantine plus caller rejection. The
visible manual Modbus panel uses a request-scoped result queue, but remaining
synchronous program branches can still block the invoking thread. Full HMI
correctness requires a central transport service for Teensy commands, with
typed command-specific response schemas and explicit cancellation semantics.
